...
export default function AuthPage() {
  ...
  const handleAuthErrorResponse = (errorData: any, type: "login" | "signup") => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    setLoginErrors(null);
    try {
      const response = await fetch("https://golden-goblin-master.ngrok-free.app/api/auth/login", {
        method: "POST",
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://golden-goblin-master.ngrok-free.app/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json", "ngrok-skip-browser-warning": "1" },
              ...
              <Tabs defaultValue={activeTab} onValueChange={handleTabSwitch}>
                ...
                <TabsContent value="login">
                  <Card>
                    ...
                    {loginErrors && <div className="text-red-500 text-lg text-center">{loginErrors}</div>}
                      <form onSubmit={handleLoginSubmit}>
                        <div className="space-y-2">
                          <Label htmlFor="email">Email</Label>
                          <Input id="email" type="email" placeholder="m@example.com" value={loginData.email} onChange={(e) => handleInputChange(e, "login")} required />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="password">Password</Label>
                          <Input id="password" type="password" value={loginData.password} onChange={(e) => handleInputChange(e, "login")} required />
                        </div>
                        <CardFooter>
                ...
                <TabsContent value="signup">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Create a new account to start exchanging books.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                    {signupErrors && <div className="text-red-500 text-lg text-center">{signupErrors}</div>}
                      <form onSubmit={handleSignupSubmit}>
                        <div className="space-y-2">
                          <Label htmlFor="first_name">First Name</Label>
                          <Input id="first_name" placeholder="John" value={signupData.first_name} onChange={(e) => handleInputChange(e, "signup")} required />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="last_name">Last Name</Label>
                          <Input id="last_name" placeholder="Doe" value={signupData.last_name} onChange={(e) => handleInputChange(e, "signup")} required />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="email">Email</Label>
                          <Input id="email" type="email" placeholder="m@example.com" value={signupData.email} onChange={(e) => handleInputChange(e, "signup")} required />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="password">Password</Label>
                          <Input id="password" type="password" value={signupData.password} onChange={(e) => handleInputChange(e, "signup")} required />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="phone">Phone</Label>
                          <Input id="phone" type="tel" placeholder="+1 (555) 555-5555" value={signupData.phone} onChange={(e) => handleInputChange(e, "signup")} required />
                        </div>
                        <CardFooter>
        ...
        <AlertDialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                You are successfully {authType === "login" ? "logged in" : "registered"}!
